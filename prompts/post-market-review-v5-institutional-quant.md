...
# 七十九、GitHub Pages 公开分发契约

复盘完成验证、生成不可变报告并通过原发布流程后，允许独立的 Presentation / Distribution Layer 将**最终 HTML 收盘复盘**同步到统一研究门户：
...
Pages 只负责公开阅读、导航和历史归档，不重新计算 Actual、Proper Scoring、Regime、Risk/PnL、Model Health 或 Next-Day Context。Pages 发布失败只能记为 `PAGES_STATUS=FAILED`，不得把已经成功的收盘复盘标为失败。

## 79.1 允许公开

仅允许发布经过原流程正式生成、存在 `publication.json`、通过哈希核对且模式不是 `test` 的最终 `report.html`。晨报与复盘必须发布到同一个站点。

## 79.2 禁止公开

不得进入 Pages：

- DeepSeek、Wind、GitHub 或其他凭据；
- `.env`、DPAPI 文件、HTTP 头、账户信息和本地绝对路径；
- Wind 原始响应、原始行情数据库、日志、SQLite、Evidence Pack；
- Forecast、Actual、Validation、Feature Snapshot、Next-Day Context 等内部 JSON；
- Markdown、DOCX 或其他未进入公开白名单的附件。

LLM 不得决定是否发布、不得生成 Pages 路径、不得修改历史报告或部署配置。公开同步由确定性代码按 allowlist 执行。
...
```text
published-reports/YYYY/MM/YYYYMMDD/review.html
```

`latest/review.html` 必须复制最新成功归档的复盘，不使用符号链接。若当日复盘尚未生成，应继续保留最近一次成功复盘；完全没有复盘时显示“今日收盘复盘尚未生成”，不得返回 404。历史报告必须跨部署保留。
